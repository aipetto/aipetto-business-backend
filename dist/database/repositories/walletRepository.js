...
class WalletRepository {
    ...
    static update(id, data, options) {
        return __awaiter(this, void 0, void 0, function* () {
            const currentTenant = mongooseRepository_1.default.getCurrentTenant(options);
            let record = yield mongooseRepository_1.default.wrapWithSessionIfExists(wallet_1.default(options.database).findOne({ _id: id, tenant: currentTenant.id }), options);
            if (!record) {
    ...
    static findAndCountAll({ filter, limit = 0, offset = 0, orderBy = '' }, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (filter) {
                ...
                if (filter.totalCreditsRange) {
                    const [start, end] = filter.totalCreditsRange;
                ...
                if (filter.aipettoPointsRange) {
                    const [start, end] = filter.aipettoPointsRange;
                    if (start !== undefined && start !== null && start !== '') {
                        criteriaAnd.push({
                            aipettoPoints: {
                                $gte: start,
                ...
                if (filter.user) {
                    criteriaAnd.push({
                        user: mongooseQueryUtils_1.default.uuid(filter.user),
                    });
                }
    ...
    static _createAuditLog(action, id, data, options) {
        return __awaiter(this, void 0, void 0, function* () {
            yield auditLogRepository_1.default.log({
                entityName: wallet_1.default(options.database).modelName,
                entityId: id,
                action,
                values: data,
            }, options);
    ...
    static _mapRelationshipsAndFillDownloadUrl(record) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const output = record.toObject
                ? record.toObject()
                : record;
            output.user = userRepository_1.default.cleanupForRelationships(output.user);
            return output;
        });
    }
}
