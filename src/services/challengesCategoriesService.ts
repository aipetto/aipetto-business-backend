...
export default class ChallengesCategoriesService {
  ...
  async destroyAll(ids) {
    ...
    try {
      for (const id of ids) {
        await ChallengesCategoriesRepository.destroy(id, {
          ...this.options,
          session,
        });
      }
  ...
  async findById(id) {
    return ChallengesCategoriesRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return ChallengesCategoriesRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return ChallengesCategoriesRepository.findAndCountAll(
      args,
      this.options,
    );
  }
  ...
  async _isImportHashExistent(importHash) {
    const count = await ChallengesCategoriesRepository.count(
      {
        importHash,
      },
      this.options,
