...
export default class BusinessPlaceServiceAvailabilityService {
  ...
  async destroyAll(ids) {
    ...
    try {
      for (const id of ids) {
        await BusinessPlaceServiceAvailabilityRepository.destroy(id, {
          ...this.options,
          session,
        });
      }
  ...
  async findById(id) {
    return BusinessPlaceServiceAvailabilityRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return BusinessPlaceServiceAvailabilityRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return BusinessPlaceServiceAvailabilityRepository.findAndCountAll(
      args,
      this.options,
    );
  }
  ...
  async _isImportHashExistent(importHash) {
    const count = await BusinessPlaceServiceAvailabilityRepository.count(
      {
        importHash,
      },
      this.options,
