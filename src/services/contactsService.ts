...
export default class ContactsService {
  ...
  async findById(id) {
    return ContactsRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return ContactsRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return ContactsRepository.findAndCountAll(
      args,
      this.options,
    );
  }
  ...
  async _isImportHashExistent(importHash) {
    const count = await ContactsRepository.count(
      {
        importHash,
      },
      this.options,
