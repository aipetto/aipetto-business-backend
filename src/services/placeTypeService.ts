...
export default class PlaceTypeService {
  ...
  async destroyAll(ids) {
    ...
    try {
      for (const id of ids) {
        await PlaceTypeRepository.destroy(id, {
          ...this.options,
          session,
        });
      }
  ...
  async findById(id) {
    return PlaceTypeRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return PlaceTypeRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return PlaceTypeRepository.findAndCountAll(
      args,
      this.options,
    );
  }
  ...
  async _isImportHashExistent(importHash) {
    const count = await PlaceTypeRepository.count(
      {
        importHash,
      },
      this.options,
