...
class LanguagesRepository {
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await MongooseRepository.wrapWithSessionIfExists(
      Languages(options.database).findOne({_id: id, tenant: currentTenant.id}),
      options,
    );
    ...
    await Languages(options.database).updateOne(
      { _id: id },
      {
        ...data,
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await Languages(options.database).deleteOne({ _id: id }, options);
  ...
  static async filterIdsInTenant(
    ...
    const records = await Languages(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      Languages(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    let rows = await Languages(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort);

    const count = await Languages(
      options.database,
  ...
  static async _createAuditLog(action, id, data, options: IRepositoryOptions) {
    await AuditLogRepository.log(
      {
        entityName: Languages(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
