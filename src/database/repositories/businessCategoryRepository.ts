...
class BusinessCategoryRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await BusinessCategory(
      options.database,
    ).create(
      [
        {
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    await BusinessCategory(options.database).updateOne(
      { _id: id },
      {
        ...data,
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await BusinessCategory(options.database).deleteOne({ _id: id }, options);
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      BusinessCategory(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    let rows = await BusinessCategory(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort)
    ...
    const count = await BusinessCategory(
      options.database,
  ...
  static async _createAuditLog(action, id, data, options: IRepositoryOptions) {
    await AuditLogRepository.log(
      {
        entityName: BusinessCategory(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
...
export default BusinessCategoryRepository;
