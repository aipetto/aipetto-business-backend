...
class BusinessPlaceServiceAvailabilityRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await BusinessPlaceServiceAvailability(
      options.database,
    ).create(
      [
        {
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    await BusinessPlaceServiceAvailability(options.database).updateOne(
      { _id: id },
      {
        ...data,
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await BusinessPlaceServiceAvailability(options.database).deleteOne({ _id: id }, options);
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      BusinessPlaceServiceAvailability(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    let rows = await BusinessPlaceServiceAvailability(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort)
    ...
    const count = await BusinessPlaceServiceAvailability(
      options.database,
  ...
  static async _createAuditLog(action, id, data, options: IRepositoryOptions) {
    await AuditLogRepository.log(
      {
        entityName: BusinessPlaceServiceAvailability(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
...
export default BusinessPlaceServiceAvailabilityRepository;
