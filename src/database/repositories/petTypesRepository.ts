...
class PetTypesRepository {
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await MongooseRepository.wrapWithSessionIfExists(
      PetTypes(options.database).findOne({_id: id, tenant: currentTenant.id}),
      options,
    );
  ...
  static async filterIdsInTenant(
    ...
    const records = await PetTypes(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      PetTypes(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    let rows = await PetTypes(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort)
  ...
  static async _createAuditLog(action, id, data, options: IRepositoryOptions) {
    await AuditLogRepository.log(
      {
        entityName: PetTypes(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
...
export default PetTypesRepository;
