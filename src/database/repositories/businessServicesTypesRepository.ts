...
class BusinessServicesTypesRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await BusinessServicesTypes(
      options.database,
    ).create(
      [
        {
  ...
  static async update(id, data, options: IRepositoryOptions) {
    ...
    let record = await MongooseRepository.wrapWithSessionIfExists(
      BusinessServicesTypes(options.database).findOne({_id: id, tenant: currentTenant.id}),
      options,
    );
    ...
    await BusinessServicesTypes(options.database).updateOne(
      { _id: id },
      {
        ...data,
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await BusinessServicesTypes(options.database).deleteOne({ _id: id }, options);
  ...
  static async filterIdsInTenant(
    ...
    const records = await BusinessServicesTypes(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      BusinessServicesTypes(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    let rows = await BusinessServicesTypes(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort)
  ...
  static async _createAuditLog(action, id, data, options: IRepositoryOptions) {
    await AuditLogRepository.log(
      {
        entityName: BusinessServicesTypes(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
...
export default BusinessServicesTypesRepository;
